...
def generateModelData(
    ...
    """Generate Cook's membrane-shaped grid of material points.
    ...
    h0
        The height left of the body.
    h1
        The height right of the body.
    nX
    ...
    for i in range(nX):
        for j in range(nY):
            ...
            mpVolume = 0.5 * (quad_h1 + quad_h2) * dx * thickness 
            mpCoordinates = np.array([np.mean(quad, axis=0)])
            #mpCoordinates_old = grid[:, i, j].reshape(-1, 2)
