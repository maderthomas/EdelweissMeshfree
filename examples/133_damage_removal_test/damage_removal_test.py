...
from edelweissmeshfree.particlemanagers.removedamagedparticlemanager import (
    RemoveDamagedParticleManager,
)
...
def run_sim():
    ...
    nX = 30
    nY =10
    supportRadius = 1.5 * np.sqrt((length / nX) ** 2 + (height / nY) ** 2)
    ...
    #}
    theMaterial = {
            "material": "GMCDPFiniteStrain",
            "properties": np.array(
            [
                #E, nu, GcToG, lb, lt, lj2, polarRatio, fcu, fcy, ftu, fbu,
                30000, 0.2, 1, 1, 1.99, 1, 1.499, 55., 16.5, 3.6, 63.8,
                #Df, softeningModulus, maxDamage, nonLocalRadius, weightingParameter,
                0.85, 1.1e-1, 0.99, 1, 1,
                #Ah, Bh, Ch, Dh, As,
                0.08, 0.003, 2.0, 1e-6, 15,
                #a1, a2, a3, a4,
                0.5, 0.0, 0.5, 0.0])}
    ...
    #theParticleManager = RemoveDamagedParticleManager(
    #    particleKernelDomain = theParticleKernelDomain,
    #    dimension = dimension,
    #    journal = theJournal,
    ...
    #        "left", theModel.particleSets["rectangular_grid_left"], "displacement", {0: 0, 1: 0}, theModel, location="center"
    #)
    #dirichletRight = ParticleLagrangianWeakDirichletOnParticleSetFactory(
    #        "right", theModel.particleSets["rectangular_grid_right"], "displacement", {0: 20}, theModel, location="center"
    #)
    dirichletBottomLeft = ParticleLagrangianWeakDirichletOnParticleSetFactory(
            "bottom_left", theModel.particleSets["rectangular_grid_leftBottom"], "displacement", {0:0, 1: 0}, theModel, location="center"
    )
    dirichletBottomRight = ParticleLagrangianWeakDirichletOnParticleSetFactory(
            "bottom_right", theModel.particleSets["rectangular_grid_rightBottom"], "displacement", {0:0, 1: 0}, theModel, location="center"
    )
    #dirichletTop = ParticleLagrangianWeakDirichletOnParticleSetFactory(
    #        "top", theModel.particleSets["rectangular_grid_top"], "displacement", {1: 5}, theModel, location="center"
    #)
    #dirichletTopRight = ParticleLagrangianWeakDirichletOnParticleSetFactory(
    #    "top_right",
    ...
    theModel.constraints.update(dirichletBottomRight)
    #theModel.constraints.update(dirichletBottomRight)
    #theModel.constraints.update(dirichletTop)
    #theModel.constraints.update(dirichletTopRight)
    ...
    incSize = 1e-1
    adaptiveTimeStepper = AdaptiveTimeStepper(0.0, 1.0, incSize, incSize, 1e-8, 100, theJournal)
    ...
    iterationOptions["max. iterations"] = 30
    ...
    try:
        nonlinearSolver.solveStep(
            ...
            #particleDistributedLoads=[pressure_top],
            particleDistributedLoads=[pressure_top],
